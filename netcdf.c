...
int writeNetCDFFile(const char * netcdfFile, int width, int height, int *ecef, type_variables var)
{
   ...
   int ncid, ncid_group1, ncid_group2;
   ...
   if((retval = nc_create(netcdfFile, NC_NETCDF4, &ncid))) {
    ...
    // NC DEF GROUP
   if((retval = nc_def_grp(ncid, "IJ", &ncid_group1))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   if((retval = nc_def_grp(ncid, "XYZ", &ncid_group2))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   ...
   for(int i=0; i<var.nb_bands; i++) {
    ...
    if((retval = nc_def_var(ncid_group1, name, NC_INT, 2,
            dimids_ij, &varid_colors[i]))) {
        fprintf(stderr, "Error: %s\n", i, nc_strerror(retval));
        exit(2);
    }
   }
   if((retval = nc_def_var(ncid_group1, "nb_views", NC_INT, 2,
            dimids_ij, &varid[0]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   if((retval = nc_def_var(ncid_group1, "nodata", NC_INT, 2,
            dimids_ij, &varid[1]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   if((retval = nc_def_var(ncid_group1, "radius", NC_FLOAT, 2,
            dimids_ij, &varid[2]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   ...
   for(int v=0; v<nbView; v++) {
        ...
        if((retval = nc_def_var(ncid_group2, name, NC_INT, 3,
   ...
   if((retval = nc_def_var(ncid_group2, "coord_ECEF", NC_DOUBLE, 3,
   ...
   int nvars_in_grp1 = var.nb_bands + nbr_variables + (nbView-1);
   int nvars_in_grp2 = 2*nbView + 1;
   int *list_varid_group1 = malloc(nvars_in_grp1*sizeof(int));
   int *list_varid_group2 = malloc(nvars_in_grp2*sizeof(int));

   int cpt = 0;
   for(int v=0; v<nbr_variables; v++) {
        list_varid_group1[++cpt] = varid[v];
   }
   for(int v=0; v<var.nb_bands; v++) {
        list_varid_group1[++cpt] = varid_colors[v];
   }
   for(int v=0; v<nbView-1; v++) {
        list_varid_group1[++cpt] = varid_view[v];
   }

   for(int v=0; v<2*nbView; v++) {
        list_varid_group2[v] = varid_dir_view[v];
   }
   list_varid_group2[2*nbView] = varid_ecef;


   if((retval = nc_inq_varids(ncid_group1, &nvars_in_grp1, list_varid_group1))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   fprintf(stdout, "------ CPT = %d ---------\n", cpt);

   if((retval = nc_inq_varids(ncid_group2, &nvars_in_grp2, list_varid_group2))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   ...
   for(int i=0; i<var.nb_bands; i++) {
    ...
    if((retval = nc_put_var_int(ncid_group1, varid_colors[i], &bands[0]))) {
        fprintf(stderr, "Error: %s\n", i, nc_strerror(retval));
        exit(2);
    }
   }
   if((retval = nc_put_var_int(ncid_group1, varid[0], &var.nbViews[0]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   if((retval = nc_put_var_int(ncid_group1, varid[1], &var.nodata[0]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   if((retval = nc_put_var_float(ncid_group1, varid[2], &var.radiusMap[0]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
   ...
   for(int v=0; v<nbView-1; v++) {
        ...
        if((retval = nc_put_var_int(ncid_group1, varid_view[v], &view[0]))) {
            fprintf(stderr, "Error: %s\n", nc_strerror(retval));
            exit(2);
        }
   }
   ...
   for(int v=0; v<nbView; v++) {
       ...
       if((retval = nc_put_var_int(ncid_group2, varid_dir_view[v], &dir_view[0]))) {
        fprintf(stderr, "Error: %s\n", nc_strerror(retval));
        exit(2);
       }
       ...
       if((retval = nc_put_var_int(ncid_group2, varid_dir_view[v+nbView], &dir_view[0]))) {
        fprintf(stderr, "Error: %s\n", nc_strerror(retval));
        exit(2);
       }
   }

   if((retval = nc_put_var_int(ncid_group2, varid_ecef, &ecef[0]))) {
    fprintf(stderr, "Error: %s\n", nc_strerror(retval));
    exit(2);
   }
