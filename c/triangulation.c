...
// compute the ball size of the intersection
double get_intersection_radius(type_sight *sights_list, int N, double *point_opt)
{
    double radius = 0;
    double pts1[3], vect1[3];
    double pts2[3], vect2[3];
    type_sight *sights_list_new = (type_sight *) malloc(N*sizeof(type_sight));
    double *outerr = (double *) malloc(2*sizeof(double));
    double point_opt_tmp[3];
    double new_radius;

    for(int i=0; i<N; i++) {
        memcpy(pts1, sights_list[i].s, sizeof(sights_list[i].s));
        memcpy(vect1, sights_list[i].v, sizeof(sights_list[i].v));
        for(int j=i+1; j<N; j++) {
            memcpy(pts2, sights_list[j].s, sizeof(sights_list[j].s));
            memcpy(vect2, sights_list[j].v, sizeof(sights_list[j].v));
            //fprintf(stderr, "pts1=%f %f %f\n", pts1[0], pts1[1], pts1[2]);
            //fprintf(stderr, "pts2=%f %f %f\n", pts2[0], pts2[1], pts2[2]);
            memcpy(sights_list_new[0].s, pts1, sizeof(pts1));
            memcpy(sights_list_new[1].s, pts2, sizeof(pts2));
            memcpy(sights_list_new[0].v, vect1, sizeof(vect1));
            memcpy(sights_list_new[1].v, vect2, sizeof(vect2));
            find_point_opt(sights_list_new, 2, point_opt_tmp, outerr);
            VEC_SCALE(point_opt_tmp, -1, point_opt_tmp);
            VEC_SUM(point_opt_tmp, point_opt, point_opt_tmp);
            VEC_LENGTH(new_radius, point_opt_tmp);
...
double find_point_opt(type_sight *sights_list, int N,
            double *point_opt, double *outerr)
{
    ...
    // compute radius
    if(N > 2)
        return get_intersection_radius(sights_list, N, point_opt);
    else
        return 0.0;
}
...
double rpc_height_geo(list_of_pairs *list_pairs,
              int local_nb_sights,
              type_sight *sights_list, double* radius)
{
    // build the set of sights
    ...
    *radius = find_point_opt(sights_list, local_nb_sights,
               point_opt, outerr);
