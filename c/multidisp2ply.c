...
int main_disp_to_heights(int c, char *v[])
{
  ...
  // check the output format desired
  bool netcdf = pick_option(&c, &v, "-netcdf", NULL);
  ...
  for (int y = 0; y < height; y++)
   ...
   // Generation of a 3D image
      ...
      int *ecef_coords = malloc(3*width*height*sizeof(int));
      type_variables var;
      var.nb_sights = nb_sights;
      //fprintf(stdout, "nb_sights=%d\n", nb_sights);
      var.nb_bands = pd; // 3 bands : R G and B
      var.colors = malloc(var.nb_bands*width*height*sizeof(int));
      var.nbViews = malloc(width*height*sizeof(int));
      var.views = malloc(var.nb_sights*width*height*sizeof(int));
      var.nodata = malloc(width*height*sizeof(int));
      var.dir_view_x_origin = malloc(var.nb_sights*width*height*sizeof(int));
      var.dir_view_y_origin = malloc(var.nb_sights*width*height*sizeof(int));
      var.dir_view_z_origin = malloc(var.nb_sights*width*height*sizeof(int));
      var.dir_view_x_vector = malloc(var.nb_sights*width*height*sizeof(int));
      var.dir_view_y_vector = malloc(var.nb_sights*width*height*sizeof(int));
      var.dir_view_z_vector = malloc(var.nb_sights*width*height*sizeof(int));
      var.radiusMap = malloc(width*height*sizeof(float));

      // initialisation
      for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            var.nbViews[x + width*y] = 0;
            var.nodata[x + width*y] = 0;
            var.radiusMap[x + width*y] = NAN;
            for(int t=0; t<var.nb_sights; t++)
            {
                var.views[var.nb_sights*y*width + var.nb_sights*x + t] = 1;
                var.dir_view_x_origin[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
                var.dir_view_y_origin[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
                var.dir_view_z_origin[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
                var.dir_view_x_vector[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
                var.dir_view_y_vector[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
                var.dir_view_z_vector[var.nb_sights*y*width + var.nb_sights*x + t] = 0;
            }
            for(int t=0; t<3; t++)
            {
                ecef_coords[width*3*y+3*x+t] = 0;
            }
            for(int t=0; t<var.nb_bands; t++)
            {
                var.colors[width*var.nb_bands*y+var.nb_bands*x+t] = 0;
            }
        }
      }
      ...
      for (int y = 0; y < height; y++) {
        ...
        for(int pid=0;pid<list_pairs.real_size;pid++)
          {
            ...
            if (isnan(posx) || isnan(posy)) {
                list_pairs.data[pid].process = false;
                local_nb_sights --;
                var.views[var.nb_sights*x + var.nb_sights*y*width + pid]--;
                var.nodata[posH]++;
            }
            else
            {
        ...
        if(clr) {
            for(int t=0; t<var.nb_bands; t++)
                var.colors[var.nb_bands*posH + t] = (int) clr[var.nb_bands*posH + t];
        }

        var.nbViews[posH] = local_nb_sights;
        ...
        if (local_nb_sights >= 2)
          {
            ...
            // - each sight is related to the optimal point, so we only need the first one.
            for(int t=0; t<3; t++) {
              ecef_coords[posH+(t*width*height)] = (int) sights_list[0].err_vec_ptopt_to_sight[t];
            }

            for(int t=0; t<local_nb_sights; t++)
            {
                var.dir_view_x_origin[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[0];
                var.dir_view_y_origin[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[1];
                var.dir_view_z_origin[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[2];
                var.dir_view_x_vector[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[0] - sights_list[t].p[0];
                var.dir_view_y_vector[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[1] - sights_list[t].p[1];
                var.dir_view_z_vector[var.nb_sights*y*width + var.nb_sights*x + t] = (int) sights_list[t].s[2] - sights_list[t].p[2];
            }

            // * radius
            var.radiusMap[posH] = (float) radius;
            npoints ++;
       ...
       // * save images
       char buf[1000];
       sprintf(buf, "%s/cloud3D_image.nc", tile_dir);
       writeNetCDFFile(buf, width, height, ecef_coords, var);
       ...
       free(var.views);
       free(var.colors);
       free(var.dir_view_x_origin);
       free(var.dir_view_y_origin);
       free(var.dir_view_z_origin);
       free(var.dir_view_x_vector);
       free(var.dir_view_y_vector);
       free(var.dir_view_z_vector);
       free(var.radiusMap);
   }
